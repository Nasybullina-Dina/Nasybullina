...
int count_tankers(int* adj, unsigned int width, unsigned int height, int* big)
{
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            ...
            if (!visited[idx] && adj[idx]) {
                sz = dfs(x, y, width, height, NULL, adj, visited, NULL, NULL, dfs_count_size|dfs_use_adj);
                if (sz > max_sz) {
                    max_sz = sz;
                    best_x = x;
                    best_y = y;
    ...
    for (i = 0; i < width*height; i++)
        visited[i] = 0;
    if (max_sz > 0)
        dfs(best_x, best_y, width, height, NULL, adj, visited, big, NULL, dfs_mark_big|dfs_use_adj);
    for (i = 0; i < width*height; i++)
        visited[i] = 0;
    int count = 0, idx;
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            ...
            if (!visited[idx] && adj[idx]) {
                int ok = 1;
                dfs(x, y, width, height, NULL, adj, visited, big, &ok, dfs_use_adj|dfs_count_size);
                if (ok)
                    count++;
            }
        }
