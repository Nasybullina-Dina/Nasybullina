...
{ 
    ...
    unsigned char* picture = (unsigned char*)malloc(size*sizeof(unsigned char));
    ...
    write_png("intermediate_result.png", result, width, height);
    color(blr_pic, result, bw_size);
    
    // выписали результат
    write_png("picture_out.png", result, width, height);
